...
class PointModel:
    ...
    def train_and_predict_hours(self, day, Xy_train, Xy_test, predictions_list, inverse_predictions):
        """
        Train the model and make predictions for each hour in the unique_hours, 
        and store the predictions in a list.
        ...
        for hour in self.unique_hours:
            ...
            Xy_train_hour, y_train_hour = self.fit_transform_data(Xy_train_hour[hour_variables + self.y_column])
            X_test_hour = self.fit_transform_data(X_test_hour[hour_variables], is_train=False)
            ...
            if len(Xy_train_hour) > 0 and len(X_test_hour) > 0:
                ...
                if (len(prediction) > 0) and inverse_predictions:
                    _, prediction_df = self.transformation_pipeline.inverse_transform(yt=prediction[0])
    ...
    def predict(self, rolling_window=728, inverse_predictions=True):
        ...
        new_predictions_df = pd.DataFrame(predictions_list, columns=['DateTime', f'prediction_{rolling_window}rw'])
