...
class PITScaler(BaseScaler):
    ...
    def __init__(self, distribution='normal', nu=8):
        ...
        if distribution not in valid_distributions:
            raise ValueError(f"Invalid distribution type. Use one of {valid_distributions}.")
    ...
    def _inverse_transform_columns(self, X):
        ...
        for column in X.columns:
            ...
            inv_ecdf = interp1d(empirical_cdf, inverted_pit, bounds_error=False, assume_sorted=True)
    ...
    def _inverse_transform_y(self, y, ref_column):
        ...
        inv_ecdf = interp1d(sorted_transformed_data, sorted_original_data, bounds_error=False, assume_sorted=True)
        return inv_ecdf(y)
